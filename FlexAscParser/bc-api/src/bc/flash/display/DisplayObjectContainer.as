package bc.flash.display
{
	...
	public class DisplayObjectContainer extends InteractiveObject
	{
		...
		private static var sListenersList : Vector.<DisplayObject> = new Vector.<DisplayObject>();

		private static var sAddedEvent : Event = new Event(Event.ADDED, true);
		private static var sRemovedEvent : Event = new Event(Event.REMOVED, true);
		private static var sAddedToStageEvent : Event = new Event(Event.ADDED_TO_STAGE);
		private static var sRemovedFromStageEvent : Event = new Event(Event.REMOVED_FROM_STAGE);
		...
		{
			var childsCount : int = numChildren;
		...
		public function addChildAt(child : DisplayObject, index : int) : void
		{
			...
			if (index >= 0 && index <= numChildren)
			{
				...
				if (stage) child.dispatchEventOnChildren(sAddedToStageEvent);
			}
			else
			{
		...
		public function removeChildAt(index : int, dispose : Boolean = false) : void
		{
			if (index >= 0 && index < numChildren)
			{
				...
				if (stage) child.dispatchEventOnChildren(sRemovedFromStageEvent);
		...
		public override function hitTest(localPoint : Point, forTouch : Boolean = false) : DisplayObject
		{
			...
			var childsCount : int = numChildren;
			for (var i : int = childsCount - 1; i >= 0; --i) // front to back!
			{
		...
		internal override function dispatchEventOnChildren(event : Event) : void
		{
			...
			getChildEventListeners(this, event.type, sListenersList);
			var numListeners : int = sListenersList.length;

			for (var i : int = 0; i < numListeners; ++i)
				sListenersList[i].dispatchEvent(event);

			sListenersList.length = 0;
		}
		...
		{
			return mChildren == null ? 0 : int(mChildren.length);
		}
	}
}
